          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div
                  className="testimonial-card-glow relative overflow-hidden group p-5 md:p-6 rounded-2xl border border-border bg-card/40 shadow-lg shadow-primary/5 max-w-xs w-full backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1"
                  key={i}
                >
                  ...
                  <div className="relative z-10 flex flex-col justify-between h-full">
                    ...
                    <div className="flex items-center gap-2 mt-4">
                      ...
                      <div className="flex flex-col">
                        <div className="font-bold tracking-tight leading-5 text-xs font-serif uppercase text-foreground lines-card-title">{name}</div>
