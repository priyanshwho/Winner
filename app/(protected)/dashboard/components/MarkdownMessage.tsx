  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h1: ({ children }) => <h1 className="text-lg font-bold mb-2.5 text-foreground">{children}</h1>,
        h2: ({ children }) => <h2 className="text-base font-bold mb-2 text-foreground">{children}</h2>,
        h3: ({ children }) => <h3 className="text-base font-semibold mb-1.5 text-foreground/90">{children}</h3>,
        ...
        code: ({ children, className }) => {
          ...
          return isBlock ? (
            <code className="block bg-muted/40 border border-border/80 rounded-lg px-3 py-2 my-2 text-sm font-mono text-primary overflow-x-auto whitespace-pre">{children}</code>
          ) : (
            <code className="bg-muted border border-border/30 rounded px-1 py-0.5 text-sm font-mono text-primary">{children}</code>
          );
        },
